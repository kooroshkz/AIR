...
class GlobalModelState:
    def __init__(self) -> None:
        ...
        #also makes for easier visualizations for the object, which will be useful when building a more intuitive ui for them
    ...
    def set_norm_percentiles(self, lower : float | None, upper : float | None):
        #also check ranges for this one, expecially since its setting a raneg for percentiles

        if not lower and not upper:
            return

        if "norm_percentiles" not in self.__attributes:
            self.__attributes["norm_percentiles"] = {"lower" : 1.0, "upper" : 99.0}

        #HUGE SMELL, NEED TO ADD CHECK FOR BOUNDS OF LOWER AND HIGHER, AND THAT THEY DONT OVERLAP
        #WILL ADD THIS WHEN I IMPLEMENT THE PROPER SETTERS WITH DECORATORS FOR ALL THE ATTRIBUTES
        if lower:
    ...
    def set_selected_model(self, model):
        ...
        #models have a path
        #models seem to have something called "diam_labels" and "diam_mean"? (find out what these are)
    ...
    def __bool__(self):
        ...
        #meaning that the biologist hasnt yet finalized what their final pipeline stage will look like
        pass
