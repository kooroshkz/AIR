...
class ModelInterface(QWidget):
    ...
    def __init__(self, viewer, filter_widget):
        ...
        self.main_window = main(self) #the QMainWindow object representing the Cellpose UI
    ...
    def _launch_cellpose_ui(self) -> None:
        #get the pipeline which final model will be attached to (if a pipeline is selected)
        ModelInterface.preprocessing_pipeline = self.filter_widget.workflow.get_pipelines()

        #3 options, user just closed it, user picked to launch with no pipeline, or user selected pipeline
        if ModelInterface.preprocessing_pipeline is None:
            return
        elif ModelInterface.preprocessing_pipeline.name == "no pipeline":
            self.filter_widget.add_to_chat(f"[Info] launching cellpose with no pipeline attached")
            ModelInterface.pipeline_attached = False
            ModelInterface.preprocessing_pipeline = None
        else:
            self.filter_widget.add_to_chat(f"[Info] launching cellpose with pipeline {ModelInterface.preprocessing_pipeline.name}")
