...
class CellposeNapariWidget(QWidget):
    ...
    def run_segmentation(self):
        ...
        try:
            ...
            # Unpack only what we need, allowing for extra return values in newer cellpose versions
            ...
            self.status_label.setText(f"Segmentation complete: Found {len(np.unique(masks))-1} cells")
            
            # Show preview in Napari
            self.viewer.add_labels(masks, name=f"{layer_name}_cellpose_preview", opacity=0.5)
    ...
    def apply_to_napari(self):
        ...
        if self.masks is not None:
            ...
            self.viewer.add_labels(self.masks, name=f"{layer_name}_cellpose_masks")
            
            # adds flow layers for visualization(No idea if this is actually useful or not)
            if self.flows is not None and len(self.flows) > 0:
                self.viewer.add_image(self.flows[0], name=f"{layer_name}_flows_y", colormap='blues')
                self.viewer.add_image(self.flows[1], name=f"{layer_name}_flows_x", colormap='reds')
                pass
...
class CellposeLaunchPoint(QWidget):
    ...
    def _launch_cellpipe(self) -> None:
        ...
        if not CELLPOSE_AVAILABLE:
            self.filter_widget.add_to_chat("[ERROR] Cellpose is not installed. Please install Cellpose to use this feature.")
            return
        ...
        if self.cellpose_widget is None:
            ...
            self.filter_widget.add_to_chat("[INFO] Cellpose interface launched in Napari")
        else:
            ...
            self.filter_widget.add_to_chat("[INFO] Reopened Cellpose interface in Napari")
