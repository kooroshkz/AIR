...
def test_class_2D(data_dir, image_names):
    ...
    chan2 = [1, 0, 0]
    diams = [30., 30., 15.]
    shapes = [(*img.shape[:2], 1), (*img.shape[:2], 1),
    ...
    for m, model_type in enumerate(model_types):
        ...
        io.imsave(str(data_dir.joinpath("2D").joinpath(
            f"gray_2D_{model_type}.tif")), img_restore)
    clear_output(data_dir, image_names)
...
def test_dn_cp_class_2D(data_dir, image_names):
    ...
    chan2 = [1, 0, 0]
    diams = [30., 30., 15.]
    shapes = [(*img.shape[:2], 2), (*img.shape[:2], 1),
              (img.shape[0] * 2, img.shape[1] * 2, 1)]
    for m, model_type in enumerate(model_types):
        model = denoise.CellposeDenoiseModel(
            model_type="cyto3",
            restore_type=model_type,
            chan2_restore=True)
        ...
        io.imsave(str(data_dir.joinpath("2D").joinpath(
            f"rgb_2D_{model_type}.tif")), img_restore)
    clear_output(data_dir, image_names)
