...
from qtpy.QtWidgets import QFileDialog, QFormLayout, QScrollArea, QMainWindow, QApplication, QSizePolicy, QStackedWidget, QVBoxLayout, QWidget, QScrollBar, QComboBox, QGridLayout, QPushButton, QFrame, QCheckBox, QLabel, QProgressBar, QLineEdit, QMessageBox, QGroupBox
...
def make_bwr():
    # make a bwr colormap
    b = np.append(255 * np.ones(128), np.linspace(0, 255, 128)
                  [::-1])[:, np.newaxis]
...
class MainW(QMainWindow):
    ...
    def __init__(self, image=None, logger=None):
        ...
        # the stack widget containing
        # - the scrollarea which holds the model interface
        # - the scrollarea which holds the pipeline interface
        self.stackwidget = QStackedWidget()

        # --- the scroll area containing the model/training-loop UI --- #
        ...
        # --- the scroll area containing the pipeline UI --- #
        self.scrollareaPipeline = QScrollArea()
        self.scrollareaPipeline.setVerticalScrollBarPolicy(
            QtCore.Qt.ScrollBarAlwaysOn)
        self.scrollareaPipeline.setStyleSheet(
            """QScrollArea { border: none }""")
        self.scrollareaPipeline.setWidgetResizable(True)
        self.swidgetPipeline = QWidget(self)
        self.scrollareaPipeline.setWidget(self.swidgetPipeline)
        self.l1 = QVBoxLayout()

        self.swidgetPipeline.setLayout(self.l1)
        b1 = self.make_buttons_pipeline()

        self.stackwidget.addWidget(self.scrollarea)
        self.stackwidget.addWidget(self.scrollareaPipeline)

        # --- switch views between model & pipeline interface --- #
        self.switchViewBtn = QPushButton("switch view")
        self.switchViewBtn.clicked.connect(self.switch_view)
        self.switchViewBtn.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Fixed)

        self.lmain.addWidget(self.stackwidget, 0, 0, 39, 9)
        self.lmain.addWidget(self.switchViewBtn)
        ...
        for i in range(3):
            self.cmap.append(
                make_cmap(i).getLookupTable(
                    start=0.0, stop=255.0, alpha=False))

        if MATPLOTLIB:
            self.colormap = (plt.get_cmap("gist_ncar")(
                np.linspace(0.0, .9, 1000000)) * 255).astype(np.uint8)
    ...
    def switch_view(self):
        current_index = self.stackwidget.currentIndex()

        # Switch to the other widget
        if current_index == 0:
            self.stackwidget.setCurrentIndex(1)  # Switch to scroll area
        else:
            self.stackwidget.setCurrentIndex(0)  # Switch to label
    ...
    def make_buttons(self):
        ...
        self.DeleteMultipleROIButton.clicked.connect(
            self.delete_multiple_cells)
        ...
        self.CancelDeleteMultipleROIButton.clicked.connect(
            self.cancel_remove_multiple)
        self.deleteBoxG.addWidget(
            self.CancelDeleteMultipleROIButton, 2, 2, 1, 2)
        ...
        self.ChannelChoose[0].addItems(
            ["0: gray", "1: red", "2: green", "3: blue"])
        self.ChannelChoose[1].addItems(
        ...
        for p in range(2):
            ...
            self.segaBoxG.addWidget(
                self.norm_edits[p], b0, 4 * (p % 2) + 2, 1, 2)
        ...
        self.ModelChooseC.activated.connect(
            lambda: self.model_choose(custom=True))
        ...
        self.ModelChooseB.activated.connect(
            lambda: self.model_choose(custom=False))
        ...
        self.save_norm.setToolTip(
            "save restored/filtered image in _seg.npy file")
    ...
    def make_buttons_pipeline(self):
        self.boldfont = QtGui.QFont("Arial", 11, QtGui.QFont.Bold)
        self.boldmedfont = QtGui.QFont("Arial", 9, QtGui.QFont.Bold)
        self.medfont = QtGui.QFont("Arial", 9)
        self.smallfont = QtGui.QFont("Arial", 8)

        # --- menu explorer for importing pipelines --- #
        self.pipelineBox = QGroupBox("Select Pipelines")
        self.pipelineBox.setFont(self.boldfont)
        self.pipelineBoxL = QVBoxLayout()
        self.pipelineBox.setLayout(self.pipelineBoxL)
        self.pipelineBox.minimumSizeHint()

        importBtn = QPushButton("Import pipeline")
        importBtn.setToolTip("import a pipeline from a file")
        importBtn.clicked.connect(self.import_pipeline)
        self.pipelineBoxL.addWidget(importBtn)

        self.l1.addWidget(self.pipelineBox)

        # --- current pipeline info box --- #
        ...
        self.pipelineInfoL.addRow("Name: ", QLabel("example name"))
        self.pipelineInfoL.addRow("number of functions: ", QLabel("69"))
    ...
    def import_pipeline(self):
        ...
        if file_path:
            try:
                ...
                # until better error handling is done
        ...
        if self.loaded:
            if not (event.modifiers() &
                ...
                if len(self.current_point_set) > 0:
                    ...
                    nviews += int(
                        self.ViewDropDown.model().item(
                            self.ViewDropDown.count() -
                            1).isEnabled())
        ...
        if index > 0:
            ...
            print(
                f"GUI_INFO: diameter set to {
                    self.diameter: 0.2f} (but can be changed)")
    ...
    def make_viewbox(self):
        self.p0 = guiparts.ViewBoxNoRightDrag(
            parent=self, lockAspect=True, name="plot1", border=[
                100, 100, 100], invertY=True)
    ...
    def clear_all(self):
        ...
        if self.restore and "upsample" in self.restore:
            ...
            self.cellpix_orig = np.zeros(
                (self.NZ, self.Ly0, self.Lx0), np.uint16)
            self.outpix_orig = np.zeros(
                (self.NZ, self.Ly0, self.Lx0), np.uint16)
        else:
            self.layerz = 0 * np.ones((self.Ly, self.Lx, 4), np.uint8)
    ...
    def unselect_cell(self):
        if self.selected > 0:
            ...
            if idx < self.ncells + 1:
                ...
                if self.outlinesOn:
                    self.layerz[self.outpix[z] == idx] = np.array(
    ...
    def remove_region_cells(self):
        ...
        # make roi region here in center of view, making ROI half the size of
        # the view
        ...
        roi = pg.RectROI(
            pos, [
                roi_width, roi_height], pen=pg.mkPen(
                "y", width=2), removable=True)
    ...
    def merge_cells(self, idx):
        ...
        if self.selected != self.prev_selected:
            for z in range(self.NZ):
                ...
                self.draw_mask(
                    z, ar, ac, vr, vc, color, idx=self.prev_selected)
            self.remove_cell(self.selected)
    ...
    def remove_stroke(self, delete_points=True, stroke_ind=-1):
        ...
        if inZ:
            ...
            if self.outlinesOn:
                self.layerz[stroke[outpix, 1], stroke[outpix, 2]
                            ] = np.array(self.outcolor)
            if delete_points:
    ...
    def plot_clicked(self, event):
        if event.button() == QtCore.Qt.LeftButton and not event.modifiers() & (
                QtCore.Qt.ShiftModifier | QtCore.Qt.AltModifier) and not self.removing_region:
            if event.double():
                try:
    ...
    def roi_changed(self, roi):
        ...
        # deselect cells not in region by deselecting all and then selecting
    ...
    def update_plot(self):
        ...
        if self.view == 0 or self.view == self.ViewDropDown.count() - 1:
            ...
            if len(self.flows) >= self.view - \
                    1 and len(self.flows[self.view - 1]) > 0:
    ...
    def add_set(self):
        if len(self.current_point_set) > 0:
            ...
            if len(self.current_point_set[0]) > 8:
                ...
                median = self.add_mask(
                    points=self.current_point_set, color=color)
    ...
    def compute_scale(self):
        ...
        self.radii = np.zeros(
            (self.Ly + self.radii_padding, self.Lx, 4), np.uint8)
        ...
        if self.masksOn:
            ...
            if len(inZ) > 0:
                for i in inZ:
                    ...
                    self.layerz[stroke[:, 1], stroke[:, 2]
                                ] = np.array([255, 0, 255, 100])
        else:
            self.layerz[..., 3] = 0
    ...
    def set_normalize_params(self, normalize_params):
        ...
        percentile = self.check_percentile_params(
            normalize_params["percentile"])
            ...
            if np.ptp(img_norm[..., c]) > 1e-3:
                if norm3D:
                    ...
                    for z in range(self.NZ):
                        if self.NZ > 1:
                            x01 = np.percentile(
                                img_norm[z, :, :, c], percentile[0])
                            x99 = np.percentile(
                                img_norm[z, :, :, c], percentile[1])
                        else:
                            x01 = np.percentile(
                                img_norm[..., c], percentile[0])
                            x99 = np.percentile(
                                img_norm[..., c], percentile[1])
                        if invert:
                            x01i = 255. - x99
                            x99i = 255. - x01
                            x01, x99 = x01i, x99i
    ...
    def initialize_model(self, model_name=None, custom=False):
        ...
        if model_name is None or not isinstance(model_name, str):
            ...
            if self.current_model == "cyto" or self.current_model == "nuclei":
                self.current_model_path = models.model_path(
    ...
    def train_model(self, restore=None, normalize_params=None):
        ...
        if self.training_params["model_index"] < len(models.MODEL_NAMES):
            ...
            self.logger.info(
                f"training new model starting at model {model_type}")
        else:
            model_type = None
        ...
        self.logger.info(
            f"training with chan = {
                self.ChannelChoose[0].currentText()}, chan2 = {
        ...
        print("GUI_INFO: name of new model: " +
              self.training_params["model_name"])
        ...
        self.logger.info(
            f"!!! computed masks for {
                os.path.split(
                    self.filename)[1]} from new model !!!")
    ...
    def compute_denoise_model(self, model_type=None):
        ...
        try:
            ...
            if "upsample" in model_name:
                ...
                print(
                    f"GUI_INFO: upsampling image to {diam_up} pixel diameter ({
            ...
            for c in range(img_norm.shape[-1]):
                ...
                for z in range(self.NZ):
                    x01 = np.percentile(
                        img_norm[z, :, :, c], percentile[0]) * 255.
                    x99 = np.percentile(
                        img_norm[z, :, :, c], percentile[1]) * 255.
            ...
            if model_type == "upsample":
                ...
                outlines = masks_to_outlines(self.cellpix_resize[0])[
                    np.newaxis, :, :]
        ...
        try:
            ...
            if self.load_3D:
                if stitch_threshold == 0.:
                    flows_new.append(
                        (flows[1][0] / 10 * 127 + 127).astype("uint8"))
                else:
                    flows_new.append(
                        np.zeros(
                            flows[1][0].shape,
