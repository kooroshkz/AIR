...
def _extend_centers_gpu(neighbors, meds, isneighbor, shape, n_iter=200,
    ...
    if T.ndim == 2:
        ...
        del grads
        mu_torch = np.stack(
            (dy.cpu().squeeze(0), dx.cpu().squeeze(0)), axis=-2)
    else:
...
def masks_to_flows(masks, device=torch.device("cpu"), niter=None):
    ...
    if masks.ndim == 3:
        ...
        for z in range(Lz):
            mu0 = masks_to_flows_device(
                masks[z], device=device, niter=niter)[0]
            mu[[1, 2], z] += mu0
        for y in range(Ly):
            mu0 = masks_to_flows_device(
                masks[:, y], device=device, niter=niter)[0]
            mu[[0, 2], :, y] += mu0
        for x in range(Lx):
            mu0 = masks_to_flows_device(
                masks[:, :, x], device=device, niter=niter)[0]
            mu[[0, 1], :, :, x] += mu0
        return mu
    elif masks.ndim == 2:
...
def labels_to_flows(
    ...
    if labels[0].shape[0] == 1 or labels[0].ndim < 3 or redo_flows:
        ...
        labels = [fastremap.renumber(label, in_place=True)[0]
                  for label in labels]
        iterator = trange if nimg > 1 else range
        for n in iterator(nimg):
            labels[n][0] = fastremap.renumber(labels[n][0], in_place=True)[0]
            vecn = masks_to_flows(
                labels[n][0].astype(int),
                device=device,
                niter=niter)

            # concatenate labels, distance transform, vector flows, heat
            # (boundary and mask are computed in augmentations)
...
def steps_interp(dP, inds, niter, device=torch.device("cpu")):
    ...
    if (device.type == "cuda" or device.type == "mps") or ndim == 3:
        pt = torch.zeros((*[1] * ndim, len(inds[0]), ndim),
        ...
        for n in range(ndim):
            if ndim == 3:
                pt[0, 0, 0, :, ndim - n - \
                    1] = torch.from_numpy(inds[n]).to(device, dtype=torch.float32)
            else:
                pt[0, 0, :, ndim - n - \
                    1] = torch.from_numpy(inds[n]).to(device, dtype=torch.float32)
            im[0, ndim - n -
                1] = torch.from_numpy(dP[n]).to(device, dtype=torch.float32)
...
def remove_bad_flow_masks(
    ...
    device0 = device
    if masks.size > 10000 * \
            10000 and (device is not None and device.type == "cuda"):
        ...
        if masks.size * 32 > free_mem:
            ...
            dynamics_logger.info(
                "turn off QC step with flow_threshold=0 if too slow")
...
def get_masks_torch(pt, inds, shape0, rpad=20, max_size_fraction=0.4):
    ...
    for k in range(n_seeds):
        slc = tuple([slice(seeds1[k][j] - 5, seeds1[k][j] + 6)
                    for j in range(ndim)])
        h_slc[k] = h1[slc]
    del h1
    ...
    if resize is not None:
        if len(resize) == 2:
            ...
            if mask.shape[0] != Lz or mask.shape[1] != Ly:
                dynamics_logger.info(
                    "resizing 3D masks to original image size")
                if mask.shape[1] != Ly:
                    mask = transforms.resize_image(
                        mask, Ly=Ly, Lx=Lx, no_channels=True, interpolation=cv2.INTER_NEAREST)
                if mask.shape[0] != Lz:
                    mask = transforms.resize_image(
                        mask.transpose(
                            1,
...
def compute_masks(dP, cellprob, p=None, niter=200, cellprob_threshold=0.0,
    ...
    if (cellprob > cellprob_threshold).sum(
