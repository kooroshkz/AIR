        ...
        for p in range(3):
            ...
            if args.anisotropy > 1.0 and p > 0:
                imgs = transforms.resize_image(
                    imgs, Ly=int(args.anisotropy * Ly), Lx=Lx)
            for k, img in enumerate(imgs):
                if args.tile_norm:
                    img = transforms.normalize99_tile(
                        img, blocksize=args.tile_norm)
                if args.sharpen_radius:
                    img = transforms.smooth_sharpen_img(
                        img, sharpen_radius=args.sharpen_radius)
                ly = 0 if Ly - \
                    crop_size <= 0 else np.random.randint(0, Ly - crop_size)
                lx = 0 if Lx - \
                    crop_size <= 0 else np.random.randint(0, Lx - crop_size)
                io.imsave(os.path.join(dirname,
                                       f'train/{name0}_{npm[p]}_{k}.tif'),
                          img[ly:ly + args.crop_size,
                              lx:lx + args.crop_size].squeeze())
